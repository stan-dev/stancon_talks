data {
  ...
  int<lower=0, upper=1> art_turf[n_games];
...
model {
  ...
  sigma_y ~ normal(0,5);
  b_home ~ normal(0,1);
  art_turf_effect ~ normal(0, 5);
  sigma_a_raw ~ normal(0,1);
  tau_a ~ cauchy(0,1);
  to_vector(eta_a) ~ normal(0,1);
  // Likelihood
  for (g in 1:n_games) {
    a_diff[g] = art_turf_effect * art_turf[g] + a[home_week[g],home_team[g]] - a[away_week[g],away_team[g]];
  }
...
generated quantities {
  ...
  for (g in 1:n_games)
    goal_difference_rep[g] = student_t_rng(nu, a[home_week[g],home_team[g]] - 
                                        a[away_week[g],away_team[g]]+b_home + 
                                        art_turf_effect * art_turf[g], sigma_y);
  ...
  for (g in 1:n_games_pred)
    goal_difference_pred_rep[g] = student_t_rng(nu, a[n_weeks, home_team_pred[g]] - 
                                        a[n_weeks, away_team_pred[g]]+b_home +
                                        art_turf_effect * art_turf[g], sigma_y);                                      
}
