functions {
  ...
  real skellam_lpmf(int k, real mu1, real mu2) {
    ...
    log_prob = total + log_modified_bessel_first_kind(k_real, 2 * sqrt(mu1*mu2));
...
model {
  ...
  sigma_a0 ~ normal(0,2); // actually a half-normal
 
  sigma_a_raw ~ normal(0,1); // actually a half-normal
  tau_a ~ cauchy(0,1);
